...
export default function Team() {
	...
	const [pageData, setPageData] = useState<TeamPageData | null>(null);

	useEffect(() => {
		// Fetch team page data
		fetch('https://firuzanail.ru/api/team-page')
			.then(res => res.json())
	...
	return (
		<>
			<Head>
				<title>{pageData?.data.title || 'Наша команда'} | FirUza Nail Studio</title>
				<meta 
					name="description" 
					content={pageData?.data.description || 'Фундамент FirUza Nail Studio — это профессиональные мастера, влюблённые в свою профессию.'} 
				/>
				<meta property="og:title" content={`${pageData?.data.title || 'Наша команда'} | FirUza Nail Studio`} />
				<meta 
				...
				<section ref={firstSectionRef} className="relative overflow-hidden">
					...
					<div className="pt-[62px] md:pt-20 pb-24 md:pb-52 px-5 md:px-0 flex flex-col items-center gap-y-[36px] md:gap-y-4 mx-auto md:max-w-[760px] text-center">
						<TextReveal className="uppercase text-white font-tenor-sans text-[28px] md:text-[64px]">
							{pageData?.data.title || 'Наша команда'}
						</TextReveal>
						<FadeUpText delay={0.1}>
							<p className="text-white font-tilda-sans text-lg max-w-[324px] md:max-w-[516px]">
								{pageData?.data.description || 'Фундамент FirUza Nail Studio — это профессиональные мастера, влюблённые в свою профессию.'}
							</p>
						</FadeUpText>
					</div>
				</section>
